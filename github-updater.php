	...
	class NerdPress_GHU_Core
	{
		...
		function get_github_updates() {
			...
			foreach ( $plugins as $plugin_path => $info ) {
				...
				if ( 'runguard-support/runguard-support.php' !== $plugin_path ) {
					continue;
				}
			...
			if ( 'plugin_information' == $action ) {
				foreach ( $this->update_data as $plugin_path => $info ) {
					if ( $info['slug'] == $args->slug ) {
					  ...
					  $changelog_url = 'https://api.github.com/repos/runbotco/runguard-support/releases';
					  ...
					  if ( ! is_wp_error( $changelog ) && wp_remote_retrieve_response_code( $changelog ) == '200' ) {
						...
						$changelog_output = '<a href="https://github.com/runbotco/runguard-support/releases" target="_blank">View the changelog here</a>.';
					  }
					  ...
					  return (object) array(
						  ...
						  'banners' => array(
							  'low' => 'https://runguard.co/wp-content/uploads/runguard-support-plugin-header-small.jpg',
							  'high' => 'https://runguard.co/wp-content/uploads/runguard-support-plugin-header-large.jpg'
						  )
					  );
					}
				}
